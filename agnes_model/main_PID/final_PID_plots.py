...
def plot_PID(tb, pw, condition):
    ...
    cols=list(pref_dics.get(pw).values())
    cols.remove('Time')
    lines = plot_table.plot.line(title=title,style='.-', x='Time', y=cols ,color = ['royalblue', 'red', 'forestgreen','purple', 'orange', 'yellow'])
    lines.set_ylabel('Bits')
    lines.figure.savefig(path, bbox_inches="tight")
    lines.figure.show()
...
def plot_PID_p1off(tb, pw, condition):
    ...
    cols.remove('Time')
    lines = plot_table.plot.line(title=title,style='.-', x='Time', y=cols, color = ['royalblue', 'orangered', 'forestgreen','goldenrod'])
    lines.set_ylabel('Bits')
    lines.figure.savefig(path, bbox_inches="tight")
    lines.figure.show()
